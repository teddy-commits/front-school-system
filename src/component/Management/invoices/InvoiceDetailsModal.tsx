...
const InvoiceDetailsModal: React.FC<InvoiceDetailsModalProps> = ({ invoice, onClose }) => {
  ...
  const handleDownload = async () => {
    const result = await financeApi.downloadInvoice(invoice.id);
    if (result.success) {
      ...
      <div className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b sticky top-0 bg-white">
          ...
          <div className="flex space-x-2">
            ...
            <button onClick={handleDownload} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg" title="Download PDF">
          ...
          <div>
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {invoice.fees?.map((fee: any, index: number) => (
                  <tr key={index}>
