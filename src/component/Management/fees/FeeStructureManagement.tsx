...
const FeeStructureManagement: React.FC = () => {
  ...
  const fetchFeeStructures = async () => {
    setIsLoading(true);
    const result = await financeApi.getAllFeeStructures() as ApiResponse<FeeStructure[]>;
  ...
  const filterFees = () => {
    ...
    if (selectedType !== 'ALL') {
      filtered = filtered.filter(f => f.feeType === selectedType);
    }
          ...
          <div className="flex justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {formatCurrency(feeStructures.reduce((sum, f) => sum + (f.amount || 0), 0))}
              </p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="flex flex-wrap gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by description, fee type..."
              value={searchTerm}
          ...
          >
            {feeTypes.map(type => <option key={type} value={type}>{type === 'ALL' ? 'All Types' : type}</option>)}
          </select>
          <button 
